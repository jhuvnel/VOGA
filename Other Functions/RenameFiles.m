...
function RenameFiles(str1,str2,Path)
%% Handle Inputs
if nargin < 2
...
for i = 1:length(all_files)  
    ...
    elseif strcmp(ext,'.fig') 
        ...
        if ~isempty(annot)
            plot_str = get(annot,'String');
            if any(cellfun(@iscell,plot_str))
                plot_str = vertcat(plot_str{:});
            end
            if any(contains(plot_str,strrep(str1,'-',' ')))         
                for j = 1:length(annot)
                    annot(j).String = strrep(annot(j).String,strrep(str1,'-',' '),strrep(str2,'-',' '));
                end
                savefig(fig,fname);
            end
        end
