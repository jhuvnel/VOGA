...
%         RE_previous = RE_present_new(:,i-1);
    ...
    SineData(1,1) = Lthetad;
    SineData(1,2) = Rthetad;
...
%%
sumfig = figure;
set(sumfig, 'Position', get(0,'Screensize') - [0 0 0 80]);
sp1 = subplot(5,1,1);
plot(chair_data(:,6),chair_data(:,3),'k');
ylabel('Stimulus')


sp2 = subplot(5,1,2);
plot(vog_data(1:find(LE_Points(1,:), 1, 'last'),1),LE_Points(:,1:find(LE_Points(1,:), 1, 'last')));
hold on
plot(vog_data(1:find(RE_Points(1,:), 1, 'last'),14),RE_Points(:,1:find(RE_Points(1,:), 1, 'last')));
legend({'LE Y1','LE Z1','LE Y2','LE Z2','LE Y3','LE Z3','RE Y1','RE Z1','RE Y2','RE Z2','RE Y3','RE Z3'},'NumColumns',2)
ylabel('Raw point YZ data')


sp3 = subplot(5,1,3);
plot(vog_data(1:find(LE_present_new(1,:), 1, 'last'),1),LE_present_new(:,1:find(LE_present_new(1,:), 1, 'last')));
hold on
plot(vog_data(1:find(RE_present_new(1,:), 1, 'last'),14),RE_present_new(:,1:find(RE_present_new(1,:), 1, 'last')));
legend({'LE Y1','LE Z1','LE Y2','LE Z2','LE Y3','LE Z3','RE Y1','RE Z1','RE Y2','RE Z2','RE Y3','RE Z3'},'NumColumns',2)
ylabel('Sorted point YZ data')


sp4 = subplot(5,1,4);
plot(LE_time,LE_pos_HVT_head);
hold on
plot(RE_time,RE_pos_HVT_head);
legend({'LE Z','LE Y', 'LE X','RE Z','RE Y','RE X'})
ylabel('Angular position data')
...
titleinfo = sprintf('%s Lref=%d Ltheta=%d Ly0=%d Lz0=%d Rref=%d Rtheta=%d Ry0=%d Rz0=%d',strrep(chairfiles{numfiles},'_CHAIR',''),Lrefindx, Lthetad,cal_data(1,1),cal_data(1,2),Rrefindx, Rthetad,cal_data(2,1),cal_data(2,2));
sgtitle(titleinfo,'Interpreter','none')
figinfo = sprintf('%s_LE_%d_%d_%d_%d_RE_%d_%d_%d_%d',strrep(chairfiles{numfiles},'_CHAIR',''),Lrefindx, Lthetad,cal_data(1,1),cal_data(1,2),Rrefindx, Rthetad,cal_data(2,1),cal_data(2,2));
saveas(sumfig,[figinfo '.png'])
saveas(sumfig,[figinfo '.fig'])