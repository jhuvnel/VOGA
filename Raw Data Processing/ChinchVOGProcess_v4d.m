...
for numfiles = 1:length(chairfiles)
    ...
    fprintf('\n:::::::::::::::::::::::::::::::::::::::\n%s\n%s\n%s',[filepaths chairfiles{numfiles}],[filepaths strrep(chairfiles{numfiles},'CHAIR','VOG')],[num2str(cal_data(1,:)) ' ' num2str(cal_data(2,:))])
...
%% Quick initial check to obtain ValidCalc array
...
%     LE_ValidCalc = nan(1,length(LE_Points));
    ...
    for i = 2:length(LE_Points)
        %         LE_previous = LE_present_new(:,i-1);
        LE_previous = LE_Points(:,Lrefindx);
        if (~LE_ValidCalc(i)) && (i > end_idx) %% only run this at the start of a "bad chunk" 
            start_idx = i;
            % Determine the end of the bad chunk
            end_idx = start_idx;
            while end_idx < length(LE_ValidCalc) && LE_ValidCalc(end_idx) == 0
                end_idx = end_idx + 1;
            end

            % Get previous good frame and first bad frame
            prev_pts = reshape(LE_Points(:, start_idx - 1), 2, 3);  %  1 col per point
            bad_pts  = reshape(LE_Points(:, start_idx), 2, 3);      %  1 col per point

            % Find which of the three dots is isolated
            [~, bad_dot] = max([min([sqrt(norm(bad_pts(:,1)-prev_pts(:,1))) sqrt(norm(bad_pts(:,1)-prev_pts(:,2))) sqrt(norm(bad_pts(:,1)-prev_pts(:,3)))])... % bad point 1
                min([sqrt(norm(bad_pts(:,2)-prev_pts(:,1))) sqrt(norm(bad_pts(:,2)-prev_pts(:,2))) sqrt(norm(bad_pts(:,2)-prev_pts(:,3)))])...% bad point 2
                min([sqrt(norm(bad_pts(:,3)-prev_pts(:,1))) sqrt(norm(bad_pts(:,3)-prev_pts(:,2))) sqrt(norm(bad_pts(:,3)-prev_pts(:,3)))])]); % bad point 3

            points = [1 2 3];
            points(points == bad_dot) = [];
            % Now do dot sorting with the other two dots
            LE_lse_array = [norm(bad_pts(:,points(1)) - prev_pts(:,1))^2 + norm(bad_pts(:,points(2)) - prev_pts(:,2))^2
                norm(bad_pts(:,points(1)) - prev_pts(:,1))^2 + norm(bad_pts(:,points(2)) - prev_pts(:,3))^2
                norm(bad_pts(:,points(1)) - prev_pts(:,2))^2 + norm(bad_pts(:,points(2)) - prev_pts(:,1))^2
                norm(bad_pts(:,points(1)) - prev_pts(:,2))^2 + norm(bad_pts(:,points(2)) - prev_pts(:,3))^2
                norm(bad_pts(:,points(1)) - prev_pts(:,3))^2 + norm(bad_pts(:,points(2)) - prev_pts(:,1))^2
                norm(bad_pts(:,points(1)) - prev_pts(:,3))^2 + norm(bad_pts(:,points(2)) - prev_pts(:,2))^2];
            [~,LE_min_perm_idx]=min(LE_lse_array);
            switch LE_min_perm_idx
                case 1
                    LE_present_new(1:2,start_idx:end_idx-1) = LE_Points((2*points(1)-1):2*points(1),start_idx:end_idx-1);
                    LE_present_new(3:4,start_idx:end_idx-1) = LE_Points((2*points(2)-1):2*points(2),start_idx:end_idx-1);
                    meany = mean([(LE_present_new(1,i)-LE_present_new(1,i-1)) (LE_present_new(3,i)-LE_present_new(3,i-1))]);
                    meanz = mean([(LE_present_new(2,i)-LE_present_new(2,i-1)) (LE_present_new(4,i)-LE_present_new(4,i-1))]);
                    %         LE_present_new(5:6,i) = LE_present_new(5:6,i-1) + [meany;meanz];
                    translation = LE_present_new(5:6,i-1) + [meany;meanz] - bad_pts(:,bad_dot);
                    LE_present_new(5:6,start_idx:end_idx-1) = translation + LE_Points((2*bad_dot-1):2*bad_dot,start_idx:end_idx-1);
                case 2
                    LE_present_new(1:2,start_idx:end_idx-1) = LE_Points((2*points(1)-1):2*points(1),start_idx:end_idx-1);
                    LE_present_new(5:6,start_idx:end_idx-1) = LE_Points((2*points(2)-1):2*points(2),start_idx:end_idx-1);
                    meany = mean([(LE_present_new(1,i)-LE_present_new(1,i-1)) (LE_present_new(5,i)-LE_present_new(5,i-1))]);
                    meanz = mean([(LE_present_new(2,i)-LE_present_new(2,i-1)) (LE_present_new(6,i)-LE_present_new(6,i-1))]);
                    translation = LE_present_new(3:4,i-1) + [meany;meanz] - bad_pts(:,bad_dot);
                    LE_present_new(3:4,start_idx:end_idx-1) = translation + LE_Points((2*bad_dot-1):2*bad_dot,start_idx:end_idx-1);
                case 3
                    LE_present_new(1:2,start_idx:end_idx-1) = LE_Points((2*points(2)-1):2*points(2),start_idx:end_idx-1);
                    LE_present_new(3:4,start_idx:end_idx-1) = LE_Points((2*points(1)-1):2*points(1),start_idx:end_idx-1);
                    meany = mean([(LE_present_new(1,i)-LE_present_new(1,i-1)) (LE_present_new(3,i)-LE_present_new(3,i-1))]);
                    meanz = mean([(LE_present_new(2,i)-LE_present_new(2,i-1)) (LE_present_new(4,i)-LE_present_new(4,i-1))]);
                    translation = LE_present_new(5:6,i-1) + [meany;meanz] - bad_pts(:,bad_dot);
                    LE_present_new(5:6,start_idx:end_idx-1) = translation + LE_Points((2*bad_dot-1):2*bad_dot,start_idx:end_idx-1);
                case 4
                    LE_present_new(3:4,start_idx:end_idx-1) = LE_Points((2*points(1)-1):2*points(1),start_idx:end_idx-1);
                    LE_present_new(5:6,start_idx:end_idx-1) = LE_Points((2*points(2)-1):2*points(2),start_idx:end_idx-1);
                    meany = mean([(LE_present_new(3,i)-LE_present_new(3,i-1)) (LE_present_new(5,i)-LE_present_new(5,i-1))]);
                    meanz = mean([(LE_present_new(4,i)-LE_present_new(4,i-1)) (LE_present_new(6,i)-LE_present_new(6,i-1))]);
                    translation = LE_present_new(1:2,i-1) + [meany;meanz] - bad_pts(:,bad_dot);
                    LE_present_new(1:2,start_idx:end_idx-1) = translation + LE_Points((2*bad_dot-1):2*bad_dot,start_idx:end_idx-1);
                case 5
                    LE_present_new(1:2,start_idx:end_idx-1) = LE_Points((2*points(2)-1):2*points(2),start_idx:end_idx-1);
                    LE_present_new(5:6,start_idx:end_idx-1) = LE_Points((2*points(1)-1):2*points(1),start_idx:end_idx-1);
                    meany = mean([(LE_present_new(1,i)-LE_present_new(1,i-1)) (LE_present_new(5,i)-LE_present_new(5,i-1))]);
                    meanz = mean([(LE_present_new(2,i)-LE_present_new(2,i-1)) (LE_present_new(6,i)-LE_present_new(6,i-1))]);
                    translation = LE_present_new(3:4,i-1) + [meany;meanz] - bad_pts(:,bad_dot);
                    LE_present_new(3:4,start_idx:end_idx-1) = translation + LE_Points((2*bad_dot-1):2*bad_dot,start_idx:end_idx-1);
        ...
        elseif (~LE_ValidCalc(i)) && ~(i > end_idx) %% only run this at the start of a "bad chunk"
        else
            LE_present = LE_Points(:,i);
            LE_lse_array = [norm(LE_previous(1:2)-LE_present(1:2))^2 + norm(LE_previous(3:4)-LE_present(3:4))^2 + norm(LE_previous(5:6)-LE_present(5:6))^2
                norm(LE_previous(1:2)-LE_present(1:2))^2 + norm(LE_previous(5:6)-LE_present(3:4))^2 + norm(LE_previous(3:4)-LE_present(5:6))^2
